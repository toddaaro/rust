...
use unwind::dwarf::DwarfUnwindStrategy;
use build::*;
use shape::*;
use type_of::*;
use common::*;
...
fn invoke(bcx: block, llfn: ValueRef, llargs: ~[ValueRef]) -> block {
    bcx.ccx().unwind_strategy.invoke(bcx, llfn, llargs)
}
...
fn trans_crate(sess: session::Session,
    ...
    let unwind_strategy = @DwarfUnwindStrategy::new() as @UnwindStrategy;
    ...
    let ccx =
        @{sess: sess,
          ...
          unwind_strategy: unwind_strategy};
