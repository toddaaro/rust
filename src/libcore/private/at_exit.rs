...
fn exit_runner(exit_fns: *ExitFunctions) unsafe {
    ...
    let exit_fns_vec = vec::from_buf(start, count as uint);
    // Run in random order to prevent accidental dependencies on
    // shutdown order
    let exit_fns_vec = rand::Rng().shuffle(exit_fns_vec);
    for exit_fns_vec.each |exit_fn| {
        let f: ~~fn() = cast::reinterpret_cast(&*exit_fn);
        match f {
            ~f => {
