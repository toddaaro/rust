...
use pipes::{Port, Chan, SharedChan, stream, ChanOne, PortOne, oneshot};
...
pub unsafe fn weaken_task(f: &fn(PortOne<ShutdownMsg>)) {
    ...
    do fn&() {
        ...
        service.try_send(UnregisterWeakTask(task));
    }
}
...
enum ServiceMsg {
    RegisterWeakTask(TaskHandle, ChanOne<ShutdownMsg>),
