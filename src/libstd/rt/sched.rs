...
impl Scheduler {
    ...
    fn interpret_message_queue(~self) -> Option<~Scheduler> {
        ...
        match this.message_queue.pop() {
            Some(PinnedTask(task)) => {
                ...
                this.change_task_context(task, Scheduler::store_stask);
                return None;
            }
            Some(Wake) => {
                this.sleepy = false;
    ...
    fn send_task_home(task: ~Task) {
        let mut task = task;
        let mut home = task.swap_unwrap_home();
        match home {
    ...
    fn resume_task_from_queue(~self) -> Option<~Scheduler> {
        ...
        match this.work_queue.pop() {
            Some(task) => {
                let mut task = task;
                let home = task.swap_unwrap_home();
                match home {
                    Sched(home_handle) => {
    ...
    pub fn terminate_current_task(~self) {
        ...
        do this.change_task_context(stask) |sched, last_task| {
            let mut last_task = last_task;
            let coroutine = last_task.coroutine.take_unwrap();
    ...
    pub fn schedule_task(~self, task: ~Task) -> Option<~Scheduler> {
        ...
        if is_home || (!homed && this.run_anything) {
            ...
            do this.change_task_context(task) |sched, last_task| {
    ...
    // Place the old task, which is the scheduler task, into the
    // sched's stask slot.
    pub fn store_stask(sched: &mut Scheduler, stask: ~Task) {
    ...
    // Yield. This is sort of .... not defined.
    pub fn yield() {
        let mut sched = Local::take::<Scheduler>();
        let stask = sched.sched_task.take_unwrap();
        sched.change_task_context(stask, Scheduler::enqueue_old);
    }
    
    // Sometimes we just want the old API though.
    
    pub fn resume_task_immediately(~self, task: ~Task) {
        self.change_task_context(task, Scheduler::store_stask);
    }
    
    pub fn resume_blocked_task_immediately(~self, blocked_task: BlockedTask) {
        match blocked_task.wake() {
            Some(task) => self.change_task_context(task, Scheduler::store_stask),
            None => Local::put(self),
        };
    }
    ...
    pub fn deschedule_running_task_and_then(~self, f: &fn(&mut Scheduler, BlockedTask)) {
        ...
        let mut this = self;
        let stask = this.sched_task.take_unwrap();
        do this.change_task_context(stask) |sched, blocked_task| {
            match BlockedTask::try_block(blocked_task) {
                Left(killed_task) => { sched.enqueue_task(killed_task) }
                Right(task) => { f(sched, task) }
            }
        }            
    }
    ...
    fn handle() {
        ...
        do run_in_bare_thread {
            ...
            let _thread_two = do Thread::start {
                ...
                let mut task = ~Task::new_root(&mut sched.stack_pool, job);
                task.death.on_exit = Some(on_exit);
                sched.bootstrap(task);
            };
        }
