    ...
    fn read(&mut self, buf: &mut [u8]) -> Result<uint, ()> {
        ...
        do Scheduler::local |scheduler| {
            ...
            do scheduler.block_running_task_and_then |task| {
                ...
                let task_cell = Cell(task);
                // FIXME: We shouldn't reallocate these callbacks every call to read
                let alloc: AllocCallback = |_| unsafe { slice_to_uv_buf(*buf_ptr) };
                do watcher.read_start(alloc) |watcher, nread, _buf, status| {

                    // Stop reading so that no read callbacks are triggered
                    // before the user calls `read` again.
                    // FIXME: Is there a performance impact to calling stop here?
                    let mut watcher = watcher;
                    watcher.read_stop();
